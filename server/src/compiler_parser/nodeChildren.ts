...
type NodeChildMapper = (node: NodeBase) => NodeBase[];

function children(...nodes: (NodeBase | VoidExpression | undefined)[]): NodeBase[] {
    return nodes.filter((node): node is NodeBase => node !== undefined && node !== voidExpression);
}

const nodeChildMappers: Partial<Record<NodeName, NodeChildMapper>> = {};

// **BNF** NAMESPACE ::= 'namespace' IDENTIFIER {'::' IDENTIFIER} '{' SCRIPT '}'
nodeChildMappers[NodeName.Namespace] = function (node) {
    const namespaceNode = node as Node_Namespace;
    return namespaceNode.script;
};

// **BNF** USING ::= 'using' 'namespace' IDENTIFIER ('::' IDENTIFIER)* ';'
nodeChildMappers[NodeName.Using] = function () {
    return [];
};
...
nodeChildMappers[NodeName.Enum] = function (node) {
...
nodeChildMappers[NodeName.Class] = function (node) {
...
nodeChildMappers[NodeName.ListPattern] = function (node) {
...
nodeChildMappers[NodeName.ListEntry] = function (node) {
...
nodeChildMappers[NodeName.Interface] = function (node) {
...
nodeChildMappers[NodeName.Var] = function (node) {
...
nodeChildMappers[NodeName.Import] = function (node) {
    const importNode = node as Node_Import;
    return [importNode.type, ...importNode.paramList.flatMap(param => children(param.type, param.defaultExpr))];
};

// **BNF** FUNCDEF ::= {'external' | 'shared'} 'funcdef' TYPE ['&'] IDENTIFIER PARAMLIST ';'
nodeChildMappers[NodeName.FuncDef] = function (node) {
    const funcDef = node as Node_FuncDef;
    return [funcDef.returnType, ...funcDef.paramList.flatMap(param => children(param.type, param.defaultExpr))];
};

// **BNF** VIRTUALPROP ::= ['private' | 'protected'] TYPE ['&'] IDENTIFIER '{' {('get' | 'set') ['const'] FUNCATTR (STATBLOCK | ';')} '}'
nodeChildMappers[NodeName.VirtualProp] = function (node) {
...
nodeChildMappers[NodeName.Mixin] = function (node) {
    const mixin = node as Node_Mixin;
    return [mixin.mixinClass];
};

// **BNF** INTERFACEMETHOD ::= TYPE ['&'] IDENTIFIER PARAMLIST ['const'] FUNCATTR ';'
nodeChildMappers[NodeName.InterfaceMethod] = function (node) {
    const intfMethod = node as Node_InterfaceMethod;
    return [intfMethod.returnType, ...intfMethod.paramList.flatMap(param => children(param.type, param.defaultExpr))];
};

// **BNF** STATBLOCK ::= '{' {VAR | STATEMENT | USING} '}'
nodeChildMappers[NodeName.StatBlock] = function (node) {
...
nodeChildMappers[NodeName.Type] = function (node) {
...
nodeChildMappers[NodeName.ForEach] = function (node) {
...
nodeChildMappers[NodeName.While] = function (node) {
...
nodeChildMappers[NodeName.DoWhile] = function (node) {
...
nodeChildMappers[NodeName.ExprTerm] = function (node) {
...
nodeChildMappers[NodeName.ConstructorCall] = function (node) {
...
nodeChildMappers[NodeName.ExprPostOp] = function (node) {
...
nodeChildMappers[NodeName.Cast] = function (node) {
...
export function getNodeChildren(node: NodeBase): NodeBase[] {
    const mapper = nodeChildMappers[node.nodeName];
    assert(mapper !== undefined);
    return mapper(node);
}
